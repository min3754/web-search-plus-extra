...
def search_exa(
    ...
    guiding_query: Optional[str] = None,
    ...
    # Build contents block
    contents: Dict[str, Any] = {
        "text": {"maxCharacters": max_characters},
    }
    highlights_obj: Dict[str, Any] = {}
    if highlights_max_chars:
        highlights_obj["maxCharacters"] = highlights_max_chars
    if guiding_query:
        highlights_obj["query"] = guiding_query
    contents["highlights"] = highlights_obj if highlights_obj else True
    if livecrawl:
    ...
    if include_text:
        body["includeText"] = [include_text] if isinstance(include_text, str) else include_text
    if exclude_text:
        body["excludeText"] = [exclude_text] if isinstance(exclude_text, str) else exclude_text
    ...
    return {
        ...
        "search_type_used": data.get("searchType"),
    }
...
Full docs: See README.md and SKILL.md
    ...
    parser.add_argument(
        "--exa-type",
        default=exa_config.get("type", "auto"),
        choices=["auto", "fast", "instant", "deep"]
    )
    parser.add_argument(
        "--category",
        choices=[
    ...
    parser.add_argument(
        "--exa-max-chars", type=int,
        default=exa_config.get("max_characters", 1000),
        help="Exa: max characters for text content (default: 1000)"
    )
    parser.add_argument(
        "--highlights-max-chars", type=int, default=None,
        help="Exa: max characters for highlights"
    )
    parser.add_argument("--guiding-query", help="Exa: guiding query for highlights relevance")
    parser.add_argument("--include-text", help="Exa: require this text in results (max 5 words)")
    parser.add_argument("--exclude-text", help="Exa: exclude results containing this text")
    parser.add_argument(
        "--additional-queries", nargs="+",
        help="Exa: additional query variations (deep search only)"
    )
    parser.add_argument(
        "--exa-livecrawl",
        choices=["never", "fallback", "preferred", "always"],
        help="Exa: live crawl mode for content freshness"
    )
    ...
    def execute_search(prov: str) -> Dict[str, Any]:
        ...
        if prov == "serper":
            ...
            return search_exa(
                ...
                include_domains=args.include_domains,
                exclude_domains=args.exclude_domains,
                max_characters=args.exa_max_chars,
                highlights_max_chars=args.highlights_max_chars,
                guiding_query=args.guiding_query,
                livecrawl=getattr(args, "exa_livecrawl", None),
    ...
    cache_context = {
        ...
        "category": args.category,
        "similar_url": args.similar_url,
        "exa_max_chars": getattr(args, "exa_max_chars", None),
        "guiding_query": getattr(args, "guiding_query", None),
