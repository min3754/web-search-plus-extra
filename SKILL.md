...
| Deep research with citations | **Exa (deep)** | `--exa-type deep -q "compare React vs Vue"` |
...
python3 scripts/search.py -p exa -q "AI regulation" --exa-type deep --max-results 5
python3 scripts/search.py -p exa -q "transformer papers" --exa-max-chars 4000 --guiding-query "key findings"
python3 scripts/search.py -p exa -q "LLM tools" --include-text "open source" --exa-livecrawl always
...
```json
{
  ...
  "exa": {"type": "auto", "max_characters": 1000},
...
```json
{
  ...
  "results": [{"title": "...", "url": "...", "snippet": "...", "score": 0.95, "image": "...", "summary": "..."}],
  "routing": {
    "auto_routed": true,
    "provider": "exa",
    "confidence": 0.78,
  ...
  "output": {"content": "...", "grounding": [{"field": "content", "confidence": "high", "citations": [...]}]},
  "search_type_used": "deep"
}
```

**Exa search types:** `auto` (default, balanced), `fast` (~450ms), `instant` (~200ms), `deep` (4-12s, highest quality with grounding citations).

**Exa options:** `--exa-max-chars` (text length), `--guiding-query` (focus highlights), `--include-text`/`--exclude-text` (filter), `--exa-livecrawl` (freshness: never/fallback/preferred/always), `--additional-queries` (deep only), `--category` (company/research paper/news/tweet/personal site/financial report/people).
